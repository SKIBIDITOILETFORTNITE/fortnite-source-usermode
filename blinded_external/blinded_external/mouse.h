...
namespace Syscalls
{
	...
	static BOOLEAN WINAPI Init()
	{
		...
		LPVOID NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(L"win32u"), "NtUserSendInput");
		...
		// windows 8.1 / windows 10
		LPVOID NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(L"win32u"), "NtUserGetAsyncKeyState");
		if (!NtUserGetAsyncKeyState_Addr)
		{
			NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(L"user32"), "NtUserGetAsyncKeyState");
			if (!NtUserGetAsyncKeyState_Addr)
			{
				// Windows 7 or lower detected
				NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(L"user32"), "GetAsyncKeyState");
				if (!NtUserGetAsyncKeyState_Addr)
					return FALSE;
			}
		}
	...
	void SendLeftClick() {
		...
		NtUserSendInput(2, inputs, sizeof(INPUT));
	}
