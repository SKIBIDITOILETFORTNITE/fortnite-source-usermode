...
BOOL CALLBACK EnumWindowsCallback(HWND hwnd, LPARAM lParam) {
    ...
    LONG ratioAllScreensX = res.x / CustomGetSystemMetrics(SM_CXSCREEN);
    LONG ratioAllScreensY = res.y / CustomGetSystemMetrics(SM_CYSCREEN);
...
#define TRANSPARENCY_COLOR RGB(0, 254, 0)
    ...
    inline int amain() {
        ...
        backBufferBmpInfo.bmiHeader.biHeight = CustomGetSystemMetrics(SM_CYSCREEN);
        backBufferBmpInfo.bmiHeader.biWidth = CustomGetSystemMetrics(SM_CXSCREEN);
        ...
        POINT res = { CustomGetSystemMetrics(SM_CXSCREEN), CustomGetSystemMetrics(SM_CYSCREEN) };
    ...
    inline HWND HiJackNotepadWindow() {
        ...
        std::vector<DWORD> existingNotepads = GetPIDs(_(L"winver.exe").decrypt());
        if (!existingNotepads.empty()) {
            for (int i(0); i < existingNotepads.size(); ++i) {
        ...
        system(_("start winver.exe")); // Start notepad, and not as child process, so easy :)

        // Finding notepad's window (we could just use FindWindow but then it would be OS language dependent)
        std::vector<DWORD> notepads = GetPIDs(_(L"winver.exe").decrypt());
        ...
        SetMenu(hwnd, NULL);
        CustomSetWindowLongA(hwnd, GWL_STYLE, WS_VISIBLE);
        CustomSetWindowLongA(hwnd, GWL_EXSTYLE, WS_EX_LAYERED | WS_EX_TRANSPARENT); // WS_EX_NOACTIVATE  and WS_EX_TOOLWINDOW removes it from taskbar

        CustomSetWindowPos(hwnd, NULL, 0, 0, CustomGetSystemMetrics(SM_CXSCREEN), CustomGetSystemMetrics(SM_CYSCREEN), SWP_SHOWWINDOW);
