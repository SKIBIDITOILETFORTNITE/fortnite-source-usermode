...
public:
	...
	FVector GetBone(int id) {
		int32_t ActiveTransforms = (read<int32_t>((uintptr_t)this + (Offsets::BoneArray + 0x48)) * std::size_t(0x10));
		auto bone_array = read<TArray1<FTransform>>((uintptr_t)this + Offsets::BoneArray + ActiveTransforms);
		auto BoneTransform = read<FTransform>(bone_array.Array + (id * 0x60));
	...
	bool IsVisible() {
		...
		double Seconds = read<double>(World + 0x148);
		float LastRenderTime = read<float>(uintptr_t(this) + 0x30C);
		return Seconds - LastRenderTime <= 0.6;
	}

	BOOL bUseAsOccluder(BOOL Value)
	{
		return write<BOOL>(uintptr_t(this) + 0x26d, Value);
	}
};
...
public:
	...
	char TeamIndex() {
		...
		return read<char>(uintptr_t(this) + Offsets::TeamIndex);
	}

	std::string GetPlayerName()
	{
		__int64 FString = read<__int64>(uintptr_t(this) + VTables::NameStructure);
		...
		Driver::ReadPhysicalMemory((PVOID)FText, NameBuffer, Length * sizeof(wchar_t));
		char v21;
		int v22;
		int i;
		int v25;
	...
	{
		uintptr_t platform_ptr = read<uintptr_t>(uintptr_t(this) + VTables::Platform);
		wchar_t platform_char[64] = { 0 };
		Driver::ReadPhysicalMemory(reinterpret_cast<PVOID>(platform_ptr), reinterpret_cast<uint8_t*>(platform_char), sizeof(platform_char));
	...
	int32_t GetRank()
	{
		uintptr_t Habenaro = read<uintptr_t>(uintptr_t(this) + VTables::HabenaroComponent);
		return read<int32_t>(Habenaro + VTables::RankProgress + 0x10);
	}
};
...
public:
	...
	FVector RelativeLocation() {
		...
		return read<FVector>(uintptr_t(this) + Offsets::RelativeLocation);
	}

	FVector Velocity() {
		return read<FVector>((uintptr_t)this + Offsets::Velocity);
	}
};
...
public:
	...
	{
		return read<EFortRarity>(uintptr_t(this) + Offsets::Rarity);
	}

	std::string WeaponName()
	{
		std::string WeaponName;
		uint64_t ItemName = read<uint64_t>(uintptr_t(this) + 0x40);
		...
		if (FLength > 0 && FLength < 50)
		{
			...
			Driver::ReadPhysicalMemory((void*)FData, (PVOID)WeaponBuffer, FLength * sizeof(wchar_t));
...
public:
	...
	AFortItemDefinition* WeaponData() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<AFortItemDefinition*>(uintptr_t(this) + Offsets::WeaponData);
	}

	BOOL IsReloadingWeapon() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<BOOL>(uintptr_t(this) + Offsets::bIsReloadingWeapon);
	}

	FLOAT GetProjectileSpeed()
	{
		return read<FLOAT>((uintptr_t)this + Offsets::ProjectileSpeed);
	}
	FLOAT GetProjectileGravity()
	{
		return read<FLOAT>((uintptr_t)this + Offsets::ProjectileGravity);
	}

	int AmmoCount() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<int>(uintptr_t(this) + Offsets::AmmoCount);
	}
};
...
public:
	...
	USkeletalMeshComponent* Mesh() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<USkeletalMeshComponent*>(uintptr_t(this) + Offsets::Mesh);
	}

	AFortWeapon* CurrentWeapon() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<AFortWeapon*>(uintptr_t(this) + Offsets::CurrentWeapon);
	}

	ARootComponent* RootComponent() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<ARootComponent*>(uintptr_t(this) + Offsets::RootComponent);
	}

	APlayerState* PlayerState() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<APlayerState*>(uintptr_t(this) + Offsets::PlayerState);
	}

	AFortItemDefinition* ItemDefinition()
	{
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<AFortItemDefinition*>(uintptr_t(this) + Offsets::PrimaryPickupItemEntry + 0x18);
	}
	...
	{
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return (read <BYTE>(uintptr_t(this) + Offsets::bIsDBNO) >> 4) & 1;
	}

	FLOAT DefaultFlyTime()
	{
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<FLOAT>(uintptr_t(this) + Offsets::DefaultFlyTime);
	}

	EFortPickupSpawnSource SpawnSourceOverride()
	{
		return read<EFortPickupSpawnSource>(uintptr_t(this) + Offsets::SpawnSourceOverride);
	}

	EFortPickupSpawnSource PickupSpawnSource()
	{
		return read<EFortPickupSpawnSource>(uintptr_t(this) + Offsets::PickupSpawnSource);
	}
};
...
public:
	...
	{
		return read<AFortPawn*>((uintptr_t)this + Offsets::PawnPrivate);
	}
};
...
public:
	...
	AFortPawn* LocalPawn() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<AFortPawn*>(uintptr_t(this) + Offsets::LocalPawn);
	}

	AFortPawn* GetTargetedPawn() {
		return read<AFortPawn*>((uintptr_t)this + Offsets::TargetedFortPawn);
	}

	void UpdateCamera() {
		auto location_pointer = read<uintptr_t>((uintptr_t)Addresses::World + 0x128);
		auto rotation_pointer = read<uintptr_t>((uintptr_t)Addresses::World + 0x138);
		...
		Addresses::Location = read<FVector>(location_pointer);
		Addresses::Rotation.X = asin(fnRot.c) * (180.0 / M_PI);
		Addresses::Rotation.Y = ((atan2(fnRot.a * -1, fnRot.b) * (180.0 / M_PI)) * -1) * -1;
		Addresses::FOVAngle = read<float>((uintptr_t)this + 0x3AC) * 90.f;
	}
...
public:
	APlayerController* PlayerController() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<APlayerController*>(uintptr_t(this) + Offsets::PlayerController);
	}
};
...
public:
	ULocalPlayers* LocalPlayers() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<ULocalPlayers*>(uintptr_t(this) + Offsets::LocalPlayer);
	}
};
...
public:
	...
	{
		return read<TArray<AFortPlayerState*>>((uintptr_t)this + Offsets::PlayerArray);
	}
...
public:
	...
	UGameInstance* OwningGameInstance() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<UGameInstance*>(uintptr_t(this) + Offsets::OwningGameInstance);
	}

	AGameStateBase* GameState() {
		if (!FN::IsValidPointer((uintptr_t)this)) return 0;
		return read<AGameStateBase*>(uintptr_t(this) + Offsets::GameState);
	}
};
