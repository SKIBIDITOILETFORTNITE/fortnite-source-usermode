...
inline void ActorCache()
{
	...
	Addresses::World = memory.Read<UWorld*>(driver::base_address + Offsets::World);
...
void WorldCache()
{
	if (Settings::Loot || Settings::LootChests)
	{
		...
		uintptr_t ItemLevels = memory.Read<uintptr_t>(World + Offsets::Levels);
		DWORD LevelCount = memory.Read<DWORD>(World + (Offsets::Levels + sizeof(PVOID)));

		for (DWORD levelIndex = 0; levelIndex < LevelCount; ++levelIndex)
		{
			uintptr_t ItemLevel = memory.Read<uintptr_t>(ItemLevels + (levelIndex * sizeof(uintptr_t)));
			DWORD ActorCount = memory.Read<DWORD>(ItemLevel + (Offsets::AActor + sizeof(PVOID)));

			uintptr_t ItemPawns = memory.Read<uintptr_t>(ItemLevel + Offsets::AActor);
			for (DWORD actorIndex = 0; actorIndex < ActorCount; ++actorIndex)
			{
				AFortPawn* ItemPawn = memory.Read<AFortPawn*>(ItemPawns + (actorIndex * sizeof(uintptr_t)));
				...
				FName ItemPawnName = memory.Read<FName>((uintptr_t)ItemPawn + 0x18);
